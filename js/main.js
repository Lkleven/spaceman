          ...
          if (waveIdx < lvDef.waves.length) {
            ...
            if (levelIdx === 1) {
              const b1 = new Boss(0),
                b2 = new Boss(0);
              b1.x = GW * 0.28;
              b1.moveTarget = { x: GW * 0.28, y: 100 };
              b2.x = GW * 0.72;
              b2.moveTarget = { x: GW * 0.72, y: 100 };
              enemies.push(b1, b2);
              floaters.push(
                new Floater(GW / 2, GH / 2 - 60, "⚠ DUAL BOSS ⚠", "#ff2200"),
              );
            } else {
              enemies.push(new Boss(levelIdx));
              floaters.push(
                new Floater(GW / 2, GH / 2 - 60, "⚠ BOSS ⚠", "#ff2200"),
              );
            }
...
function applyMusicVolume(val) {
  ...
  [bgmMenu, bgmLevel1].forEach((b) => {
    if (!b.paused) b.volume = musicVolume;
  });
  const v = Math.round(val * 100);
  [
    document.getElementById("vol-master"),
    document.getElementById("menu-vol-music"),
  ].forEach((el) => {
    if (el) el.value = v;
  });
  [
    document.getElementById("vol-master-val"),
    document.getElementById("menu-vol-music-val"),
  ].forEach((el) => {
...
function applySfxVolume(val) {
  ...
  const v = Math.round(val * 100);
  [
    document.getElementById("vol-sfx"),
    document.getElementById("menu-vol-sfx"),
  ].forEach((el) => {
    if (el) el.value = v;
  });
  [
    document.getElementById("vol-sfx-val"),
    document.getElementById("menu-vol-sfx-val"),
  ].forEach((el) => {
    if (el) el.textContent = v;
  });
}
...
["vol-master", "menu-vol-music"].forEach((id) => {
  document
    .getElementById(id)
    ?.addEventListener("input", (e) => applyMusicVolume(e.target.value / 100));
});
["vol-sfx", "menu-vol-sfx"].forEach((id) => {
  document
    .getElementById(id)
    ?.addEventListener("input", (e) => applySfxVolume(e.target.value / 100));
});
